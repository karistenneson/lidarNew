...
image(BioMass.Mod.trans)
plot(confint(BioMass.Mod.trans, estimator = 'HPM'))
####################################################
####################################################
...
PctRes <- ((predictedoutput+1) / (DATA.val.trans$STBIOMSha))*100

frame <- cbind(predictedoutput, Resoutput, PctRes, Resoutput^2, data.val [ , c('STBIOMSha', 'Forest','PlotSizeAcres', 'fpc', 'Stratum')])
colnames(frame)<- c('Prediction', 'PredictionSE', "Resoutput", "ResoutputSE","PctRes", "PctResSE", "SqResoutput", "SqResoutputSE", 'STBIOMSha', 'Forest','PlotSizeAcres', "fpc", "Stratum")
ResSqVec <- svydesign(ids = ~1, data = frame, fpc = frame$fpc, strata = frame$Stratum)

framePos <-  frame[frame$STBIOMSha > 1, ]
ResSqVecPositive <- svydesign(ids = ~1, data = framePos, fpc = framePos$fpc, strata = framePos$Stratum)

#Root Mean Square Error
sqrt(svymean(frame$SqResoutput, design = ResSqVec)) # remember to Square the Standard Error
#Root Mean Square Percent Error
sqrt(svymean((framePos$PctRes)^2, design = ResSqVecPositive)) # remember to Square the Standard Error

#MAPE
svymean(framePos$PctRes, design = ResSqVecPositive) # remember to Square the Standard Error
##############################################################################
##############################################################################
# Model diagnostics

## Assess plots with zero biomass
par(mfrow = c(2,3))
hist(data.mod$Elev_P60[data.mod$STBIOMSha<1], xlab = '60th percentile', main = 'construction data', col = 'grey')
hist(data.mod$Elev_P90[data.mod$STBIOMSha<1], xlab = '90th percentile', main = 'construction data', col = 'grey')
hist(data.mod$Pct_all_returns_above_ht[data.mod$STBIOMSha<1], xlab = 'Pct returns > 3 m', main = 'construction data', col = 'grey')

hist(data.val$Elev_P60[data.val$STBIOMSha<1], xlab = '60th percentile', main = 'validation data', col = 'grey')
hist(data.val$Elev_P90[data.val$STBIOMSha<1], xlab = '90th percentile', main = 'validation data', col = 'grey')
hist(data.val$Pct_all_returns_above_ht[data.val$STBIOMSha<1], xlab = 'Pct returns > 3 m', main = 'validation data', col = 'grey')

frame[frame$PctRes>200 & frame$STBIOMSha > 1, c(1,5,9:13)]
frame[frame$STBIOMSha<1, c(1,5,9:13)]

## Percentage
par(mfrow = c(1,1))
plot(framePos$PctRes~framePos$STBIOMSha, pch= 15)
abline(h = 100, col = 'red')

### Hex plots, weighted by sample weights
svyplot(framePos$PctRes~framePos$STBIOMSha, design = ResSqVecPositive, style = 'hex', xlab = 'Observed', ylab = 'Percent agreement')
abline(h = 100, col = 'red')

svyplot(framePos$PctRes~framePos$STBIOMSha, design = ResSqVecPositive, style = 'trans', xbins = 20, xlab = 'Observed', ylab = 'Percent agreement', ylim = c(0,200))
abline(h = 100, col = 'black')
smth <- svysmooth(framePos$PctRes~framePos$STBIOMSha, design = ResSqVecPositive)
lines(smth, col = 'red')

###################
## residuals
plot(predictedoutput~(DATA.val.trans$STBIOMSha-1), xlim = c(0,750), ylim = c(0,750), pch= 15)
abline(a = 0, b = 1, col = 'red')

unique(data.val$Forest)
par(mfrow = c(3,2))
plot(predictedoutput[data.val$Forest == 'NorthKaibab']~(DATA.val.trans$STBIOMSha[data.val$Forest == 'NorthKaibab']-1), pch= 15, xlim = c(0,650), ylim = c(0,650), main = "Kaibab")
abline(a = 0, b = 1, col = 'black')
plot(predictedoutput[data.val$Forest == 'Coconino']~(DATA.val.trans$STBIOMSha[data.val$Forest == 'Coconino']-1), pch= 15, xlim = c(0,650), ylim = c(0,650), main = "Coconino")
abline(a = 0, b = 1, col = 'black')
plot(predictedoutput[data.val$Forest == 'Tonto']~(DATA.val.trans$STBIOMSha[data.val$Forest == 'Tonto']-1), pch= 15, xlim = c(0,650), ylim = c(0,650), main = "Tonto")
abline(a = 0, b = 1, col = 'black')
plot(predictedoutput[data.val$Forest == 'Sitgreaves']~(DATA.val.trans$STBIOMSha[data.val$Forest == 'Sitgreaves']-1), pch= 15, xlim = c(0,650), ylim = c(0,650), main = "Apache-Sitgreaves")
abline(a = 0, b = 1, col = 'black')
plot(predictedoutput[data.val$Forest == 'SWJM']~(DATA.val.trans$STBIOMSha[data.val$Forest == 'SWJM']-1), pch= 15, xlim = c(0,650), ylim = c(0,650), main = "SWJM")
abline(a = 0, b = 1, col = 'black')
...
par(mfrow = c(2,3))
plot(predictedoutput[data.val$PlotSizeAcres == 0.2]~(DATA.val.trans$STBIOMSha[data.val$PlotSizeAcres == 0.2]-1), col = 'purple', pch= 15, xlim = c(0,750), ylim = c(0,750))
abline(a = 0, b = 1, col = 'black')
plot(predictedoutput[data.val$PlotSizeAcres == 0.1]~(DATA.val.trans$STBIOMSha[data.val$PlotSizeAcres == 0.1]-1), col = 'green', pch= 15, xlim = c(0,750), ylim = c(0,750))
abline(a = 0, b = 1, col = 'black')
plot(predictedoutput[data.val$PlotSizeAcres == 0.0666667]~(DATA.val.trans$STBIOMSha[data.val$PlotSizeAcres == 0.0666667]-1), col = 'blue', pch= 15, xlim = c(0,750), ylim = c(0,750))
abline(a = 0, b = 1, col = 'black')
plot(predictedoutput[data.val$PlotSizeAcres == 0.05]~(DATA.val.trans$STBIOMSha[data.val$PlotSizeAcres == 0.05]-1), col = 'orange', pch= 15, xlim = c(0,750), ylim = c(0,750))
abline(a = 0, b = 1, col = 'black')
plot(predictedoutput[data.val$PlotSizeAcres == 0.0333333]~(DATA.val.trans$STBIOMSha[data.val$PlotSizeAcres == 0.0333333]-1), col = 'red', pch= 15, xlim = c(0,750), ylim = c(0,750))
abline(a = 0, b = 1, col = 'black')
plot(predictedoutput[data.val$PlotSizeAcres == 0.02]~(DATA.val.trans$STBIOMSha[data.val$PlotSizeAcres == 0.02]-1), col = 'purple', pch= 15, xlim = c(0,750), ylim = c(0,750))


### Hex plots, weighted by sample weights
svyplot(frame$Prediction~frame$STBIOMSha, design = ResSqVec, style = 'hex', xlab = 'Observed', ylab = 'Predicted')
svyplot(frame$Prediction~frame$STBIOMSha, design = ResSqVec, style = 'trans', xbins = 20, xlab = 'Observed', ylab = 'Predicted')
abline(a = 0, b = 1, col = 'black')
smth <- svysmooth(frame$Prediction~frame$STBIOMSha, design = ResSqVec)
lines(smth, col = 'pink')
####################################################################
