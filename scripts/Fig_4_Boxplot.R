...
boxplot(dataall$MPMResidual ~ dataall$PlotSizeha, notch = T, ylim = c(-320,320), ylab = "Plot Size (ha)", 
        xlab = "Field AGB - Lidar AGB (Mg/ha)", varwidth = T, horizontal=T)

abline(v = seq(from = -300, to = 300, by = 100), col = 'gray')
abline(v= 0)

boxplot(dataall$MPMResidual ~ dataall$PlotSizeha, notch = T, add = T, col = 'light gray', varwidth = T, horizontal = T)
