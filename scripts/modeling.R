...
HPMlm <- lm(log(STBIOMS)~ Elev_CV + Elev_LCV + Elev_P80 + Pct_first_returns_above_ht + Pct_all_returns_above_ht +
              ...
              Total_all_returns + R3ERUCODE + PlotSizeAcres, data=DATA.mod)
...
FinModVar <- select(DATA.mod, Elev_LCV, Elev_P80, Pct_first_returns_above_ht,
                    All_returns_above_ht, R3ERUCODE, PlotSizeAcres)
...
BioMass.ModelF <- lm(log(STBIOMS)~ Elev_LCV + Elev_P80 + Pct_first_returns_above_ht +
                       All_returns_above_ht + R3ERUCODE + PlotSizeAcres, data=DATA.mod)
...
#A possible final model? 
FinModVar <- select(DATA.mod, Elev_skewness, Elev_P80, Pct_all_returns_above_ht,
                    Total_first_returns, R3ERUCODE, elevation, slope)

corrgram(FinModVar, order=T, lower.panel=panel.ellipse,
         upper.panel=panel.cor, text.panel=panel.txt,
         main="Lidar Predictor Data in PC2/PC1 Order") # No correlations above ~0.5

TCUFT.ModelF <- lm(log(TCUFT)~ Elev_skewness + Elev_P80 + Pct_all_returns_above_ht +
                   Total_first_returns + R3ERUCODE + elevation + slope, data=DATA.mod)
summary(TCUFT.ModelF)
