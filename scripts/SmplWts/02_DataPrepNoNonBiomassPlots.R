...
####################################################
## convert fpc from acres to # of possible 0.04 plots per strata without overlap
summary(AllData$fpc)
AllData$fpcAcres <- AllData$fpc
AllData$fpcStrataHa <- AllData$fpcAcres*0.404686
AllData$fpc04StrataHaPlot <- AllData$fpcStrataHa*25

AllData$ForestHaSize <- 1382*100 #Kaibab
AllData$ForestHaSize[AllData$Forest == 'Coconino'] <- 1136*100 #Coconino
AllData$ForestHaSize[AllData$Forest == 'Tonto'] <- 499*100 #Tonto
AllData$ForestHaSize[AllData$Forest == 'Sitgreaves'] <- 1028*100 #Sitgreaves
AllData$ForestHaSize[AllData$Forest == 'SWJM'] <- 353*100 #SWJM
AllData$ForestHaSize[AllData$Forest == 'Sitgreaves, P2'] <- 294*100 #Sitgreaves, P2
AllData$ForestHaSize[AllData$Forest == 'Apache'] <- 1700*100 #Apache

AllData$ForestHaSmplSize <- -9999 #Kaibab
AllData$ForestHaSmplSize[AllData$Forest == 'Coconino'] <- 75*100 #Coconino
AllData$ForestHaSmplSize[AllData$Forest == 'Tonto'] <- 48*100 #Tonto
AllData$ForestHaSmplSize[AllData$Forest == 'Sitgreaves'] <- 1028*100 #Sitgreaves
AllData$ForestHaSmplSize[AllData$Forest == 'SWJM'] <- -9999 #SWJM
AllData$ForestHaSmplSize[AllData$Forest == 'Sitgreaves, P2'] <- 294*100 #Sitgreaves, P2
AllData$ForestHaSmplSize[AllData$Forest == 'Apache'] <- 1700*100 #Apache
...
AllData$logmode_pctAllOver3m <- log(AllData$Elev_mode) * AllData$Pct_all_returns_above_ht
AllData$logmode_pctAlloverModeDiv1st <- log(AllData$Elev_mode) * AllData$All_returns_above_mode_div_Total_first_returns_x_100
AllData$logP01_pctAllOver3m <- log(AllData$Elev_P01) * AllData$Pct_all_returns_above_ht 
AllData$logP10_pctAllOver3m <- log(AllData$Elev_P10) * AllData$Pct_all_returns_above_ht 
AllData$logP30_pctAllOver3m <- log(AllData$Elev_P30) * AllData$Pct_all_returns_above_ht 
AllData$logP60_pctAllOver3m <- log(AllData$Elev_P60) * AllData$Pct_all_returns_above_ht 
AllData$logP90_pctAllOver3m <- log(AllData$Elev_P90) * AllData$Pct_all_returns_above_ht
...
FieldNames <- c("PLOT_ID", "X_albers", "Y_albers", "Site", "Forest", "fpcAcres", "fpcStrataHa", 'fpc04StrataHaPlot', "ForestHaSize", "ForestHaSmplSize",
                "dbhThreshcm", "PlotSizeAcres", "PlotSize", "Elev_Class", "CC_Class", "Stratum", "DOM_TYPE", "Dom_Type_Label", "Secondary_Label") #subset of field variables
Response <- c('STBIOMSha','logSTBIOMSha') 
AuxNames <- c('R3ERUCODE', 'elevation', "aspect", "slope", "NDVI_Amp", "R3ERUcodeFull", "R3ERUlabel", "R3ERUlabelName") #subset of additional environmental variables
RandUnif <- "RandomUniform" #The Random Uniform Variable
#head(select(AllData, LidarNames))

AllData$ID <- seq(1:length(AllData[,1]))
Predictors <- c('ID', FieldNames, Response, LidarNames, AuxNames)
...
data.mod <- select(DATA.mod, Predictors)
data.val <- select(DATA.val, Predictors)
data.val.ind <- select(DATA.val.ind, Predictors)
